...
int CPlateLocate::plateLocate(Mat src, vector<Mat>& resultVec, vector<RotatedRect>& resultRectVec,int index)
{
	Mat src_blur, src_gray;
	Mat grad;
	...
	for (int i = 0; i<tmp.rows; i++)
	{
		for (int j = 0; j<tmp.cols; j++)
		{
			...
			float fS = hsvSplit[1].at<uchar>(i, j) / 255.0;
			float fV = hsvSplit[2].at<uchar>(i, j) / 255.0;
			if (nH >= 200 && nH <= 255 && fS >= 0.25 && fS <= 1 && fV >= 0.3 && fV <= 1) // 蓝色
				dst_blue.at<uchar>(i, j) = 255;
			else
				dst_blue.at<uchar>(i, j) = 0;
		}
	...
	//convertScaleAbs( grad_x, grad ); 
	//cv::Mat out_blue;
	//cv::multiply(grad, dst_blue, out_blue);//蓝色和sobel结合
	//cv::Mat out_yellow;
	//cv::multiply(grad, dst_yellow, out_yellow);//黄色和sobel结合
	//if (m_debug)
	//{
	//	stringstream ss(stringstream::in | stringstream::out);
	//	ss << "image/tmp/debug_Sobel_blue" << ".jpg";
	...
	{
		stringstream ss(stringstream::in | stringstream::out);
		ss << "image/tmp/debug_detect_blue" << ".jpg";
		imwrite(ss.str(), dst_blue);
		ss.str("");
		ss << "image/tmp/debug_detect_yellow" << ".jpg";
		imwrite(ss.str(), dst_yellow);
	}

	Mat img_threshold_blue,img_threshold_yellow;
	Mat element = getStructuringElement(MORPH_RECT, Size(m_MorphSizeWidth, m_MorphSizeHeight));
	morphologyEx(dst_blue, img_threshold_blue, MORPH_CLOSE, element);//进行闭操作，先膨胀在腐蚀
	morphologyEx(dst_yellow, img_threshold_yellow, MORPH_CLOSE, element);
	...
	for (int i = 0; i< rects.size(); i++)
	{
		...
		if (verifySizes(minRect))
		{
			...
			if (angle - m_angle < 0 && angle + m_angle > 0)
			{
				...
				resultVec.push_back(resultMat);
				resultRectVec.push_back(minRect);//保存位置信息
			}
		}
	}
